// Sesión de Dashboard360 — cookie firmada con HMAC-SHA256 y scrypt para claves.
//
// Es el mismo mecanismo que usa el CRM y está copiado a propósito en vez de
// importado: son dos productos que se despliegan por separado y comparten base
// de datos por conveniencia, no por diseño. Si mañana Dashboard360 se vende
// solo, no debería arrastrar los roles ni la tabla de usuarios de un CRM que no
// va incluido. La duplicación acá es más barata que el acoplamiento.
//
// Cookie y secreto propios: una sesión del CRM no abre el tablero, y viceversa.
//
...
const COOKIE = "adoops_d360_session";
const MAX_AGE = 60 * 60 * 12; // 12 horas: una jornada completa

export type Role = "admin" | "gerente" | "analista";
...
function secret(): string {
  const s = process.env.D360_SESSION_SECRET;
  if (!s || s.length < 32) {
    throw new Error(
      "D360_SESSION_SECRET no está definida o tiene menos de 32 caracteres",
    );
  }
  return s;
}
...
/**
 * Publicar un informe al directorio es una acción con destinatario externo:
 * la reserva gerencia y admin. Un analista redacta y deja en borrador.
 */
export function puedePublicar(s: SessionData): boolean {
